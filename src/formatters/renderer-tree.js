    ...
    switch (node.type) {
      case 'branch':
        return `${openingOffset}${' '.repeat(4)}${node.name}: {\n${
          node.children
            .flatMap((child) => processNode(child, level + 1))
            .join('\n')
        }\n${closingOffset}}`;
      case 'added':
        return `${openingOffset}  + ${node.name}: ${stringify(node.currentValue, level + 1)}`;
      case 'removed':
        return `${openingOffset}  - ${node.name}: ${stringify(node.currentValue, level + 1)}`;
      case 'changed':
        return `${openingOffset}  - ${node.name}: ${stringify(node.previousValue, level + 1)}\n${openingOffset}  + ${node.name}: ${stringify(node.currentValue, level + 1)}`;
      case 'unchanged':
        return `${openingOffset}${' '.repeat(4)}${node.name}: ${stringify(node.currentValue, level + 1)}`;
      default:
        throw new Error(`Unknown node type: ${node.type}`);
    }
  };
  const renderedDiff = diff
    .flatMap((node) => processNode(node, 0))
    .join('\n');
  return `{\n${renderedDiff}\n}`;
};
